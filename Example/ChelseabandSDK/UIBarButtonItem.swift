...
extension UIBarButtonItem {
    ...
    static var moreBarButton: UIBarButtonItem {
        return .init(image: UIImage.init(named: "more"), style: .done, target: nil, action: nil)
    }

    static func backBarButton(_ target: Any?, action: Selector?) -> UIBarButtonItem {
        return .init(image: UIImage.init(named: "back"), style: .done, target: target, action: action)
    }
}
